...
#
# Source: https://github.com/splitbrain/botcheck
# Fixed: test is_json via jq itself, replacing original test w/ grep on content-type header
...
for entry in "${SOURCES[@]}"; do
  ...
  curl -fsSL -o "$tmp_body" "$url"

  is_json=0
  jq -e . "$tmp_body" >& /dev/null && is_json=1
